...
def generate_image():
    logger.info('Received POST request at /generate')
    
    # Download and prepare the dataset if needed
    # (Assuming you've already downloaded and prepared the dataset for simplicity)
    DATA_DIR = './melanoma-cancer-dataset/train'
    image_size = 64
    batch_size = 128
    latent_size = 128
    ...
    # Define the discriminator network (Assuming you've defined these elsewhere)
    class Discriminator(nn.Module):
        def __init__(self):
            super().__init__()
            self.model = nn.Sequential(
    ...
    # Define the generator network (Assuming you've defined these elsewhere)
    class Generator(nn.Module):
        def __init__(self):
            super().__init__()
            self.model = nn.Sequential(
    ...
    for epoch in range(epochs):
        for real_images, _ in train_dl:
            ...
            # Train discriminator (omitted for brevity)

            # Train generator (omitted for brevity)

        # Generate and send the image to the frontend
        if (epoch + 1) % 5 == 0:
            with torch.no_grad():
                fixed_latent = torch.randn(1, latent_size, 1, 1, device=device)
                fake_image = generator(fixed_latent).squeeze().permute(1, 2, 0).detach().cpu().numpy()
                _, encoded_img = cv2.imencode('.png', (fake_image * 255).astype('uint8'))
                binary_img = io.BytesIO(encoded_img)
                return send_file(binary_img, mimetype='image/png')
